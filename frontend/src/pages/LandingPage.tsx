  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-brand-black relative overflow-hidden">
      ...
      <div className="text-center z-10 max-w-sm w-full">
        ...
        {/* Logo */}
        <div className="mb-6">
          <p className="font-mono text-brand-green text-sm mb-2 tracking-widest">[ {t('landing.type', 'SECURITY EVENT')} ]</p>
          ...
          <h2 className="font-mono font-bold text-brand-red leading-none" style={{ fontSize: '3rem' }}>
            {t('landing.city', 'Your City')}
          </h2>
          <p className="font-mono text-brand-yellow text-lg font-bold mt-1">{t('landing.year', '2026')}</p>
        </div>
        ...
        <button
          ...
          className="w-full font-mono font-bold py-4 rounded text-black text-base uppercase tracking-widest transition-all active:scale-95 border-2 bg-brand-green border-brand-green shadow-neon hover:bg-brand-green2 hover:border-brand-green2">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3">
          <div className="border border-brand-gray2 rounded p-3 text-left">
            ...
            <p className="font-mono text-white text-xs">{t('landing.date', 'Jun 26-27, 2026')}</p>
          </div>
          <div className="border border-brand-gray2 rounded p-3 text-left">
            <p className="font-mono text-brand-green text-xs mb-1">WHERE</p>
            <p className="font-mono text-white text-xs">{t('landing.venue', 'Your Venue')}</p>
          </div>
        </div>

        {/* Recover session */}
        <p className="mt-4 font-mono text-brand-muted text-xs text-center">
          {t('landing.recover', 'Já te registaste? Abre o teu link pessoal para recuperar o passport.')}
        </p>
      </div>

      {/* Footer */}
      <p className="absolute bottom-4 font-mono text-brand-muted text-xs">
        {t('landing.footer', 'Event Passport')} · Powered by Guilherme Pires
      </p>
    </div>
  );
}
